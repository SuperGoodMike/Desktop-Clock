...
def display_weather_data(tft, weather_data, geolocation_data):
    ...
    try:
        ...
        if weather_data and 'current_weather' in weather_data:
            ...
            current_time_truncated = truncate_to_hour(current_weather['time'])
            ...
            # Unit display (text instead of images)
            unit_char = "C" if temperature_unit == "C" else "F"
            unit_x = temp_x + len(temp_text) * font.WIDTH
            tft.text(font, unit_char, unit_x, 45, gc9a01.WHITE, gc9a01.BLACK)
            ...
            time_x = center_x(time_str)
            tft.text(font, time_str, time_x, 85, gc9a01.WHITE, gc9a01.BLACK)

            # Day of the week display
            weekday = current_datetime[3]
            days = ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"]
            day_str = days[weekday]
            day_x = center_x(day_str)
            tft.text(font, day_str, day_x, 125, gc9a01.WHITE, gc9a01.BLACK)
            ...
            date_str = f"{months[month-1]} {day:02d}"
            date_x = center_x(date_str)
            tft.text(font, date_str, date_x, 165, gc9a01.WHITE, gc9a01.BLACK)
